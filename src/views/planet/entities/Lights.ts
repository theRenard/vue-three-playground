...
export default class Airplane extends Entity {
  ...
  init(): void {
    ...
    this.getSketch().getGui().add(this.moonLight.position, 'x').min(-50).max(50).step(0.001);
    this.getSketch().getGui().add(this.moonLight.position, 'y').min(-50).max(50).step(0.001);
    this.getSketch().getGui().add(this.moonLight.position, 'z').min(-50).max(50).step(0.001);
